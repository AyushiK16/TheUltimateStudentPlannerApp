...
export default class ReminderScreen extends React.Component{
  ...
  getNotifications = () => {
    ...
    db.collection('ReminderAdds').where('userId', '==', this.state.userId)
...
<ListItem.Content>
  <ListItem.Title style = {{color: 'black'}}>{item.reminderDate}</ListItem.Title>
    ...
    <View style = {{flex:1}}>
        ...
        ?(
            <View style = {styles.Subtitle}>
                <Text style = {{fontSize : 20}}>No reminders</Text>
            </View>
        )
        : (
            <SwipeableFlatlist
